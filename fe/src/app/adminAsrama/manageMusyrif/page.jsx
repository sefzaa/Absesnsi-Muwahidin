...
export default function MusyrifPage() {
    ...
    const [selectedMusyrifIds, setSelectedMusyrifIds] = useState(['']); // Array of selected musyrif IDs
    ...
    const handleOpenMusyrifModal = async (kelas) => {
        ...
        try {
            const currentIds = kelas.musyrifs.map(m => m.id_pegawai);
            setSelectedMusyrifIds(currentIds.length > 0 ? currentIds : ['']); // Default to one empty dropdown if no musyrif
    ...
    // --- ▼▼▼ FUNGSI BARU UNTUK MENGELOLA DROPDOWN MUSYRIF ▼▼▼ ---
    const handleMusyrifChange = (index, value) => {
        const updatedIds = [...selectedMusyrifIds];
        updatedIds[index] = value;
        setSelectedMusyrifIds(updatedIds);
    };

    const addMusyrifDropdown = () => {
        setSelectedMusyrifIds([...selectedMusyrifIds, '']);
    };

    const removeMusyrifDropdown = (index) => {
        const updatedIds = selectedMusyrifIds.filter((_, i) => i !== index);
        // If all dropdowns are removed, add one empty one back
    ...
    const handleAssignMusyrif = async () => {
        setIsLoadingMusyrif(true);
        // Filter out empty values and duplicates before sending
        const finalMusyrifIds = [...new Set(selectedMusyrifIds.filter(id => id))];

        try {
            const res = await fetch(`${backendUrl}/api/kelas-musyrif/${selectedKelas.id_kelas}/assign`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'x-access-token': token },
                body: JSON.stringify({ musyrifIds: finalMusyrifIds })
            });
                ...
                <div className="mt-4 max-h-96 overflow-y-auto">
                    {isLoadingSantri ? <div className="text-center"><LoaderIcon/></div> : (
                         santriList.length > 0 ? (
                            ...
                            <p className="text-center text-sm text-gray-500 italic">Belum ada santri aktif di kelas ini.</p>
                        )
                    )}
                </div>
            </Modal>

            {/* Modal Assign Musyrif (UI BARU) */}
            <Modal isOpen={isMusyrifModalOpen} onClose={() => setIsMusyrifModalOpen(false)} size="lg">
                <Dialog.Title className="text-lg font-bold text-gray-900">Atur Musyrif untuk {selectedKelas?.nama_kelas}</Dialog.Title>
                <div className="mt-4 max-h-96 overflow-y-auto pr-2 space-y-3">
                    {isLoadingMusyrif ? <div className="text-center"><LoaderIcon/></div> : (
                        <>
                            {selectedMusyrifIds.map((selectedId, index) => (
                                <div key={index} className="flex items-center gap-2">
                                    <select
                                        value={selectedId}
                                        onChange={(e) => handleMusyrifChange(index, e.target.value)}
                                          className="w-full border border-gray-300 rounded-lg p-2 focus:ring focus:ring-blue-200 flex-grow text-gray-900"
                                    >
                                    ...
                                    <button 
                                        onClick={() => removeMusyrifDropdown(index)}
                ...
                <div className="mt-6 flex justify-end gap-3 border-t pt-4">
                    <button onClick={() => setIsMusyrifModalOpen(false)} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300">Batal</button>
                    <button onClick={handleAssignMusyrif} disabled={isLoadingMusyrif} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
