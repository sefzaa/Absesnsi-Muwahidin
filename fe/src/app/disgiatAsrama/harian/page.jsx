...
// Komponen Surat Izin untuk Dicetak
const SuratIzinPrint = ({ data }) => {
    if (!data) return null;
        ...
        <div className="print-only font-sans text-black">
            ...
            <div className="w-full">
                ...
                <div className="text-center mt-4">
                    <h3 className="text-base font-bold underline">SURAT IZIN KELUAR HARIAN</h3>
                ...
                <div className="mt-4 text-xs">
                    ...
                    <table className="my-2 text-left w-full border-collapse border border-black">
                        <tbody>
                            <tr>
                                <td className="py-1 px-2 border border-black w-1/3 bg-gray-100 font-bold">Nama</td>
                                <td className="py-1 px-2 border border-black w-2/3">{data.Santri?.nama}</td>
                            </tr>
                            <tr>
                                <td className="py-1 px-2 border border-black bg-gray-100 font-bold">Kelas / Kamar</td>
                                <td className="py-1 px-2 border border-black">{data.Santri?.Kela?.nama_kelas || '-'} / {data.Santri?.Kamar?.nomor_kamar || '-'}</td>
                            </tr>
                             <tr>
                                <td className="py-1 px-2 border border-black bg-gray-100 font-bold">Keperluan</td>
                                <td className="py-1 px-2 border border-black">{data.keterangan || '-'}</td>
                            </tr>
                            <tr>
                                <td className="py-1 px-2 border border-black bg-gray-100 font-bold">Tanggal Izin</td>
                                <td className="py-1 px-2 border border-black">{moment(data.tanggal_awal).format('dddd, DD MMMM YYYY')}</td>
                            </tr>
                             <tr>
                                <td className="py-1 px-2 border border-black bg-gray-100 font-bold">Jam Keluar / Batas Kembali</td>
                                <td className="py-1 px-2 border border-black">{data.jam_keluar} WIB / {data.jam_masuk} WIB</td>
                            </tr>
                             <tr>
                                <td className="py-1 px-2 border border-black bg-gray-100 font-bold">Pamong (Penanggung Jawab)</td>
                                <td className="py-1 px-2 border border-black">{data.pamong} ({data.nama_pamong || 'Yang Bersangkutan'})</td>
                            </tr>
                        </tbody>
                    </table>

                    <p className="mt-2 text-[10px] italic">
                        * Surat ini harap dibawa dan ditunjukkan kepada petugas keamanan saat keluar dan masuk gerbang. Keterlambatan akan dikenakan sanksi sesuai peraturan yang berlaku.
                    </p>
                </div>
...
export default function HarianPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await fetch(`${backendUrl}/api/perizinan-dga/harian?${query}`, {
    ...
    const openModal = (type, data = null) => {
        ...
        if (type === 'add') {
            setFormData({
                id_izin_asrama: 2, // 2 untuk Harian
                id_santri: '',
    ...
    return (
        <>
            <div className="no-print bg-white p-6 rounded-lg shadow-sm border">
                <div className="flex justify-between items-center mb-4">
                    <button onClick={() => openModal('add')} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                        <FiPlus size={20} /><span>Tambah Izin Harian</span>
                    </button>
                ...
                {isFilterVisible && (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 p-4 border rounded-lg bg-gray-50">
                        ...
                        <div className="flex items-end">
                            <button onClick={handleClearFilters} className="w-full px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors">
                                Hapus Filter
                            </button>
                        </div>
                    </div>
                ...
                <Modal isOpen={modalState.type !== null} onClose={closeModal}>
                    {modalState.type && (
                        <>
                            <div className="flex justify-between items-center mb-6"><Dialog.Title as="h3" className="text-xl font-bold text-gray-800">{modalState.type === 'view' ? 'Detail Izin Harian' : modalState.type === 'edit' ? 'Edit Izin Harian' : 'Tambah Izin Harian Baru'}</Dialog.Title><button onClick={closeModal} className="p-1 rounded-full text-gray-400 hover:bg-gray-100"><FiX size={24} /></button></div>
                            {modalState.type === 'view' ? (
                                <div className="space-y-3 text-gray-700">
                                    <p><strong>Nama:</strong> {formData.Santri?.nama}</p><p><strong>Kelas / Kamar:</strong> {formData.Santri?.Kela?.nama_kelas || '-'} / {formData.Santri?.Kamar?.nomor_kamar || '-'}</p><p><strong>Tanggal:</strong> {moment(formData.tanggal_awal).format('DD MMMM YYYY')}</p><p><strong>Jam Keluar:</strong> {formData.jam_keluar}</p><p><strong>Jam Masuk:</strong> {formData.jam_masuk}</p><p><strong>Jam Kembali (Aktual):</strong> {formData.jam_kembali_aktual || 'Belum kembali'}</p><p><strong>Persetujuan Wali Kamar:</strong> {formData.isApprove_wk ? 'Ya' : 'Tidak'}</p><p><strong>Pamong:</strong> {formData.pamong}</p>{formData.nama_pamong && <p><strong>Nama Pamong:</strong> {formData.nama_pamong}</p>}<p><strong>Keterangan:</strong> {formData.keterangan || '-'}</p><p><strong>Status:</strong> <StatusBadge status={formData.status} /></p>
                                </div>
                            ) : (
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700">Tanggal Izin</label>
                                        <input type="date" name="tanggal_awal" value={formData.tanggal_awal || ''} onChange={(e) => { handleFormChange({ target: { name: 'tanggal_awal', value: e.target.value } }); handleFormChange({ target: { name: 'tanggal_akhir', value: e.target.value } }); }} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-gray-900" required />
                                    </div>
            ...
            <SuratIzinPrint data={dataToPrint} />
        </>
    );
}