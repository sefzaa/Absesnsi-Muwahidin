//ini adalah contoh dengan tampilan card view yang lebih modern dan 
// user-friendly dan lebhih mudah diakses di perangkat mobile
...
export default function PerizinanWaliKamarPage() {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm border space-y-6">
            ...
            {/* --- MODIFIKASI: Mengganti Tabel dengan Card View --- */}
            <PerizinanCards 
...
// --- MODIFIKASI: Komponen Card View Baru ---
const PerizinanCards = ({ records, isLoading, error, onEdit, onLaporan, onView, activeTab }) => {
    const formatDate = (dateString) => moment(dateString).format('DD MMM YYYY');

    if (isLoading) {
        return <div className="text-center p-10"><FiLoader className="animate-spin inline mr-2" /> Memuat...</div>;
    }
    ...
    if (records.length === 0) {
        return <div className="text-center p-10 text-gray-500">Tidak ada data untuk ditampilkan.</div>;
    }

    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {records.map((item) => (
                <div key={item.id_detail_izin_asrama} className="bg-white rounded-lg shadow-md border border-gray-200 p-5 flex flex-col justify-between hover:shadow-lg transition-shadow duration-300">
                    <div>
                        <div className="flex justify-between items-start mb-3">
                            <h3 className="font-bold text-gray-800 text-lg">{item.Santri.nama}</h3>
                            <StatusBadge status={item.status} />
                        </div>
                        <div className="text-sm text-gray-600 space-y-2">
                            <p><span className="font-semibold text-gray-700">Kamar:</span> {item.Santri.Kamar.nomor_kamar}</p>
                            <p><span className="font-semibold text-gray-700">Jenis Izin:</span> <span className="capitalize">{item.IzinAsrama.jenis}</span></p>
                            <p><span className="font-semibold text-gray-700">Tanggal:</span> {formatDate(item.tanggal_awal)}</p>
                            <p><span className="font-semibold text-gray-700">Jam:</span> {item.jam_keluar || '-'} / {item.jam_masuk || '-'}</p>
                        </div>
                    </div>
                    <div className="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-gray-100">
                        <button onClick={() => onView(item)} className="text-blue-500 hover:text-blue-600" title="Lihat Detail"><FiEye size={18} /></button>
                        {activeTab === 'aktif' && (
                            <>
                                <button onClick={() => onEdit(item)} className="text-yellow-500 hover:text-yellow-600" title="Edit"><FiEdit size={18} /></button>
                                <button onClick={() => onLaporan(item)} className="text-green-500 hover:text-green-600" title="Laporan Kembali"><FiFileText size={18} /></button>
                            </>
                        )}
                    </div>
                </div>
