// file: src/app/wali-kamar/tahfidz/page.jsx
"use client";
...
export default function TahfidzPage() {
    ...
    const fetchData = useCallback(debounce(async (currentFilters, authToken) => {
        ...
        try {
            const response = await fetch(`${backendUrl}/api/tahfidz-wk?${params.toString()}`, {
            ...
            if (data.length > 0 && selectedTab >= data.length) {
                setSelectedTab(0); // Reset tab jika data berubah
            }
        } catch (err) {
...
// --- Sub-Komponen untuk Halaman Tahfidz ---

const Header = ({ filters, onFilterChange }) => (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-800">Setoran Tahfidz Santri</h1>
...
const TahfidzTable = ({ records, onUpdate, onView }) => (
        ...
        <table className="w-full text-sm">
            <thead className="bg-gray-50/50 text-gray-500">
                <tr>
                    ...
                    <th className="p-4 font-medium text-left">SURAH TERAKHIR</th>
                    <th className="p-4 font-medium text-left">AYAT TERAKHIR</th>
                    <th className="p-4 font-medium text-left">AKSI</th>
                </tr>
            </thead>
            <tbody className="divide-y">
                {records.length === 0 ? (
                    <tr><td colSpan="5" className="text-center p-10 text-gray-500">Tidak ada santri di kamar ini atau sesuai filter.</td></tr>
                ) : records.map((item, index) => (
                    <tr key={item.id_santri} className="hover:bg-gray-50">
                        <td className="p-4 text-gray-600">{index + 1}</td>
                        <td className="p-4 font-semibold text-gray-800">{item.nama}</td>
                        <td className="p-4 text-gray-600">{item.lastSurah}</td>
                        <td className="p-4 text-gray-600">{item.lastAyat}</td>
                        <td className="p-4">
...
const UpdateModal = ({ isOpen, data, onClose, onSuccess, token, setNotif, setShowNotif }) => {
    const [formData, setFormData] = useState({ id_surah: '', ayat: '', tanggal: '' });
    ...
    useEffect(() => {
        if (isOpen && token) {
            fetch(`${backendUrl}/api/tahfidz-wk/options/surah`, { headers: { 'x-access-token': token }})
                .then(res => res.json())
                .then(setSurahOptions);
            setFormData({ id_surah: '', ayat: '', tanggal: new Date().toISOString().split('T')[0] });
        }
    }, [isOpen, token]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${backendUrl}/api/tahfidz-wk`, {
                method: 'POST',
    ...
    const filteredSurah = surahQuery === '' ? surahOptions : surahOptions.filter(s => s.nama.toLowerCase().includes(surahQuery.toLowerCase()));

    return (
        <Modal isOpen={isOpen} onClose={onClose}>
            <Dialog.Title as="h3" className="text-xl font-bold text-gray-800 mb-2">Update Setoran</Dialog.Title>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <Combobox name="id_surah" value={formData.id_surah} onChange={(val) => setFormData(p => ({...p, id_surah: val}))}>
                        <div className="relative">
                            <Combobox.Input 
                                ...
                                displayValue={(id) => surahOptions.find(s => s.id_surah === id)?.nama || ''} 
                                placeholder="Cari surah..."
                                required
                            />
                            <Combobox.Options className="absolute z-10 mt-1 max-h-40 w-full overflow-auto rounded-md bg-white py-1 shadow-lg">
                                {filteredSurah.map((surah) => (
                                    <Combobox.Option key={surah.id_surah} value={surah.id_surah} className={({ active }) => `cursor-pointer select-none relative py-2 pl-4 pr-4 ${active ? 'bg-blue-600 text-white' : 'text-gray-900'}`}>{surah.nama}</Combobox.Option>
                                ))}
                            </Combobox.Options>
                        </div>
                    </Combobox>
                ...
                <div>
                    ...
                    <input type="text" name="ayat" value={formData.ayat} onChange={(e) => setFormData(p => ({...p, ayat: e.target.value}))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900 focus:ring-blue-500 focus:border-blue-500" required placeholder="Contoh: 1-5 atau 10, 12, 15" />
                </div>
                <div>
...
const HistoryModal = ({ isOpen, data, onClose, token }) => {
    ...
    useEffect(() => {
        if (isOpen && data && token) {
            setIsLoading(true);
            fetch(`${backendUrl}/api/tahfidz-wk/history/${data.id_santri}`, { headers: { 'x-access-token': token }})
                .then(res => res.json())
                .then(setHistory)
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} size="max-w-2xl">
            <Dialog.Title as="h3" className="text-xl font-bold text-gray-800 mb-2">Riwayat Setoran</Dialog.Title>
            <p className="text-sm text-gray-500 mb-4">Santri: <strong>{data?.nama}</strong> ({data?.kamar})</p>
                ...
                <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500 sticky top-0">
                        <tr>
                            ...
                            <th className="p-3 font-medium text-left">Surah</th>
                            <th className="p-3 font-medium text-left">Ayat</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y">
                        {isLoading ? (
                            ...
                            history.map(item => (
                                <tr key={item.id_tahfidz}>
                                    <td className="p-3 text-gray-600">{formatDate(item.tanggal)}</td>
                                    <td className="p-3 font-semibold text-gray-800">{item.Surah.nama}</td>
                                    <td className="p-3 text-gray-600">{item.ayat}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
